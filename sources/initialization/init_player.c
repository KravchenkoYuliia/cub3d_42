 ...
 */
static bool	ft_get_player_position(t_map *map, t_player *player)
{
	int	x;
	int	y;
 ...
 */
static bool	ft_get_player_direction(t_map *map, t_player *player)
{
	char	direction;

	if (!map || !player)
		return (false);
	direction = map->grid[(int)player->pos_y][(int)player->pos_x];
	if (direction == 'N')
