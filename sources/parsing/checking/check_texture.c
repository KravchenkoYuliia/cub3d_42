...
/*   Updated: 2025/10/13 12:00:13 by jgossard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

// TODO: renamed to ft_is_texture_line
...
static void	ft_checking_path(t_cub *cub)
{
	...
	if (fd == -1)
		ft_fatal_error_in_parsing(WRONG_PATH_TEXTURE, cub);
	close(fd);
	if (ft_check_file_extension(path_to_texture, ".xpm") == false)
		ft_fatal_error_in_parsing(WRONG_EXTENSION, cub);
}
